...
const CardHome = () => {
  ...
  const [totalStockOut, setTotalStockOut] = useState<any>([]);
  ...
  useEffect(() => {
    setTotalStockOut((produk?.data ?? []).filter((item: any) => item.qty < 1));
  }, [produk]);

  return (
    <div className={`pt-5 flex justify-between gap-2 flex-wrap`}>
      ...
      <div className="w-[32%] h-[170px] bg-white rounded-2xl shadow-md p-5 flex flex-col justify-center">
        ...
        <h1 className={`text-xl font-bold pt-3`}>{totalStockOut.length}</h1>
        <p className={`text-sm pt-1`}>Total Stock Out</p>
      </div>
    </div>
  );
};
